...
function findUser(){
	...
	function state_Change(){
		if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
			...
			//获取主体内容
			//let users=result.documentElement.getElementsByTagName("user");
			let users=result.documentElement.children;
			//alert(users);
			for (var i = 0; i < users.length; i++) {
				let user=users[i];
				//获取username和age
				let username= user.getElementsByTagName("username")[0].childNodes[0].nodeValue;
				let age=user.getElementsByTagName("age")[0].childNodes[0].nodeValue;
				//新生成一行，username做为一列，age做为一列
				// 添加行
